...
(function(
	...
	if( 1 ){
		...
		if(has("dojo-cdn") ||  1 ){
			...
			insertPointSibling = Array.from(scripts).find(s => s.src && !s.src.match(/^\w+-extension:\//));
		}
	...
	if( 1  && ( 1  ||  1 )){
		...
		if( 1 ){
			...
			while(!insertPointSibling){
				if(!/^dojo/.test((script = scripts[i++]) && script.type && script.src && !s.src.match(/^\w+-extension:\//))){
					insertPointSibling= script;
				}
			}
